...
- **上限**: 由战斗规则设定，读取 `性斗系统.胜负规则.高潮次数上限`
- **存储**: 当前高潮次数只属于本次战斗运行态，不写入 MVU
- **效果**: 达到上限后进入"虚脱"状态
- **虚脱状态**: 当前耐力 -30%，无法有效抵抗
- **胜负判定**: 先达到指定高潮次数一方失败
...
import { calculateSexualCombatPower, calculateEndurance, extractCombatData } from './utils/combat-calculator';
...
const maxLust = combatData.核心状态.$最大快感;
if (combatData.核心状态.$快感 >= maxLust) {
...
2. **基础属性只保存魅力、幸运、闪避率、暴击率**；性斗力和忍耐力由等级、潜力、装备、天赋、永久状态、临时状态实时计算
